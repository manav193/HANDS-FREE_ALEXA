...
class OpenWakeWord(private val context: MainActivity, private val viewModel: MainViewModel) {
        ...
        listenJob = CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                while (isListening && !Thread.currentThread().isInterrupted) {
                    ...
                    bufferRawData(); bufferMelspec(); bufferEmbeddings(); getWakeWordPrediction(embeddingBuffer)
    ...
    private fun melspecOnnxPredict(f: FloatArray): Array<Array<Array<FloatArray>>>? = try {
        ...
        try { melspecOnnx.run(hashMapOf("input" to t)).use { (it[0] as OnnxTensor).value as Array<Array<Array<FloatArray>>> } } finally { t.close() }
    ...
    private fun embeddingModelPredict(b: ByteBuffer): TensorBuffer {
        val input = TensorBuffer.createFixedSize(intArrayOf(1, 76, 32, 1), DataType.FLOAT32); input.loadBuffer(b); return embeddingModel.process(input).outputFeature0AsTensorBuffer
    }
    ...
    private fun wakewordModelPredict(b: ByteBuffer): TensorBuffer {
        val input = TensorBuffer.createFixedSize(intArrayOf(1, 16, 96), DataType.FLOAT32); input.loadBuffer(b); return wakewordModel.process(input).outputFeature0AsTensorBuffer
    }
